    ...
    <div className="absolute inset-0 z-[60] flex items-end bg-ink/55 p-3 backdrop-blur-[2px]">
      <section className="scroll-quiet max-h-[94%] w-full overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="font-mono text-[10px] tracking-[0.16em] text-pasada-blue uppercase">
              PASADA ride receipt
            </p>
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-9 w-9 place-items-center rounded-full bg-ink-50 text-lg text-ink-500 hover:text-ink"
        ...
        <div className="mt-4 rounded-2xl bg-ink-50 p-4">
          ...
          <div className="mt-3 flex gap-3">
            <div className="mt-1 flex flex-col items-center">
              <span className="h-2.5 w-2.5 rounded-full bg-pasada-blue" />
              <span className="h-8 border-l border-dashed border-ink-300" />
              <span className="h-2.5 w-2.5 rounded-full bg-pasada-red" />
            </div>
        ...
        <div className="mt-4 divide-y divide-ink-100 rounded-2xl bg-white px-4 ring-1 ring-ink-100">
          ...
          {ride.appliedCoupon && (
            <ReceiptRow
              label="PRC coupon"
              value={`-₱${ride.appliedCoupon.discountPhp.toFixed(2)} · ${ride.appliedCoupon.status}`}
            />
          )}
          <ReceiptRow
            label="Escrow amount"
            value={`${formatBchFromSats(ride.fareSats)} BCH`}
          />
          <ReceiptRow label="PASADA rate" value={ride.config.version} />
          <ReceiptRow
            label={role === "passenger" ? "Driver" : "Passenger"}
            value={otherName}
          />
        </div>
        ...
        <div className={`mt-4 grid gap-2 ${onMessage ? "grid-cols-2" : "grid-cols-1"}`}>
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-ink-100 py-3 text-[12px] font-bold text-ink-700"
          >
            Done
          </button>
          {onMessage && (
...
function ReceiptRow({ label, value }: { label: string value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3 text-[11px]">
      ...
      <span className="max-w-[62%] truncate text-right font-medium text-ink-700">
        {value}
      </span>
    </div>
  )
