...
type View = 'apps' | 'admin'

const VIEWS: { id: View; label: string; blurb: string }[] = [
  { id: 'apps', label: 'Passenger & Driver', blurb: 'Booking, escrow, payouts' },
    ...
    <div className="min-h-screen bg-ink-50">
      <header className="sticky top-0 z-50 border-b border-ink-100 bg-white/85 backdrop-blur">
        <div className="mx-auto flex max-w-[1240px] flex-wrap items-center gap-x-6 gap-y-3 px-5 py-3">
          <div className="flex items-baseline gap-2.5">
            <span className="font-display text-lg font-black tracking-tight">PASADA</span>
            ...
            {VIEWS.map((v) => (
              <button
                ...
                className={`rounded-full px-4 py-2 font-display text-[13px] font-bold transition-colors ${
                  view === v.id ? 'bg-ink text-white' : 'text-ink-500 hover:text-ink'
                }`}
              >
                {v.label}
              </button>
          ...
          <p className="hidden text-[11px] text-ink-300 lg:block">
            {VIEWS.find((v) => v.id === view)?.blurb} · rates {fareConfig.version}
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-[1240px] px-5 py-8 lg:py-12">
        {view === 'admin' ? (
          ...
          <div className="flex flex-wrap items-start justify-center gap-8 xl:gap-12">
            <AppColumn label="Passenger application">
              <RoleAuthGate role="passenger">
                {(account) => <PassengerApp fareConfig={fareConfig} account={account} />}
              </RoleAuthGate>
            </AppColumn>
            <AppColumn label="Driver application">
              <RoleAuthGate role="driver">
                {(account) => <DriverApp fareConfig={fareConfig} account={account} />}
              </RoleAuthGate>
            </AppColumn>
          </div>
        )}
      ...
      <footer className="mx-auto max-w-[1240px] px-5 pb-10 text-[11px] leading-relaxed text-ink-300">
        Prototype. Fare amounts follow Ormoc City Ordinance No. 121, s. 2023, but PASADA measures
        the 2.5 km base distance from the passenger&apos;s pickup point rather than the Ormoc City
        Stage — an adaptation that requires validation by the appropriate city transport authority.
      </footer>
    </div>
  )
}
